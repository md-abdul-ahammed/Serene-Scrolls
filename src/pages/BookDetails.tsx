...
import {
  ...
  useGetWishlistQuery,
} from "../redux/features/personalList/listApi";
...
const BookDetails = () => {
  ...
  const handleDeleteBook = async (): Promise<void> => {
    ...
    notifications.show({
      ...
      autoClose: 3000,
      title:
        res?.data?.statusCode === 200 ? "Book Deleted" : "Operation Failed",
      message:
  ...
  useEffect(() => {
    const isWishListed = allWishlist?.data.find(
      (item: any) => item.bookId.id === bookId
    );
    const isReadingListed = allReadinglist?.data.find(
      (item: any) => item.bookId.id === bookId
    );
  ...
  const { genre, title, author, publication, addedBy }: BookType =
    data?.data || {};
    ...
    <Container size={"md"}>
      <Grid sx={{ alignItems: "center" }}>
        <Grid.Col span={4}>
          <Card.Section sx={{ display: "flex", justifyContent: "center" }}>
            <Image
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQrBstXAmkzVK-Ze6Lg_gZMVl57-7Oyvpw6QA&usqp=CAU"
              height={250}
              width={200}
            />
          </Card.Section>
      ...
      {user?.fullName && (
        <Group position="right">
          <Group spacing={8} mr={0} position="right">
            ...
            >
              <Tooltip
                label={isWish ? "Remove from wishlist" : "Add to wishlist"}
              >
                {isWish ? (
                  <IconHeartFilled size="1rem" color={"red"} />
                ) : (
            ...
            <ActionIcon
              onClick={async (): Promise<void> => {
                if (!isReading) {
                  const res: any = await addReadingList({
                    bookId,
                    userId: user?.id,
                  });
            ...
            >
              ...
              >
                {isReading ? (
                  <IconBookFilled size="1rem" color={theme.colors.yellow[6]} />
                ) : (
        ...
        >
          ...
          {allreviews?.data?.map((review: IReview) => (
            <Review
              ...
              author={review.reviewerId.fullName}
            />
          ))}
        </form>
      </Container>
      ...
      >
        ...
        <Group align="flex-end">
          <Button onClick={handleDeleteBook} color="red" disabled={isDeleting}>
